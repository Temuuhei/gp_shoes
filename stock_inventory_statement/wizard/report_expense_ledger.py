...
class report_expense_ledger(osv.osv_memory):
    ...
    _columns = {
        ...
        'pos':          fields.boolean('Pos Sale'),
        'mrp':          fields.boolean('MRP Production'),
        'consume':      fields.boolean('Internal Expense'),
    ...
    def get_export_data(self, cr, uid, ids, report_code, context=None):
        ...
        headers[0] += [u'Тоо хэмжээ', u'Худалдах үнийн дүн',u'Нийт дүн',u'Хөнгөлөлт',u'Цэвэр дүн',u'НӨТ',u'НӨТ-гүй дүн']
        widths += [4,4,4,4,4,3,4]
        if wiz['cost']:
            headers[0] += [u'Өртөг дүн']
            widths += [4]
        datas = {
            'title': u'Зарлагын товчоо тайлан',
            'headers': headers,
            'header_span': [],
            'titles': titles,
            'rows': data,
    ...
    def prepare_report_data(self, cr, uid, wiz, context=None):
        ...
        TAX_FACTOR = 0.0
        user = self.pool.get('res.users').browse(cr, uid, uid, context=context)
        if user.company_id and user.company_id.sales_taxes_id:
            for tax in user.company_id.sales_taxes_id:
        ...
        if len(location_ids)>1:
            ...
            location_tuple ="( "+ str(location_ids[0])+")"
        any_check = False
        total_dict = {'amount': 0,
                      'total': 0,
                      'qty': 0,
                      ...
                      'pure':0,
                      'cost':0}
        select = ' (select 0) AS tax, (select 0) AS discount, '
        select_name = ' p.name AS name, '
        select_origin = ' p.origin AS origin, '
        select_price = ' SUM(coalesce((m.list_price * m.product_qty / u.factor * u2.factor),0)) AS amount, '
        where_location = ' m.location_id IN (' +','.join(map(str,location_ids))+ ') AND m.location_dest_id NOT IN (' +','.join(map(str,location_ids))+ ')  '
        group_by = ''
        if wiz['product_ids']:
        ...
        if wiz['sales']:
            any_check = True
            join += ' LEFT JOIN sale_order_tax AS taxes ON (sol.id = taxes.order_line_id) '
            select = ' SUM(taxes.tax_id) AS tax, SUM(coalesce(m.product_qty * sol.price_unit * (sol.discount / 100),0)) AS discount, '
            select_price = ' (CASE WHEN po.sale_line_id is not null THEN  SUM(coalesce((sol.price_unit * m.product_qty / u.factor * u2.factor),0)) \
                            ELSE SUM(coalesce((m.list_price * m.product_qty / u.factor * u2.factor),0)) END) AS amount, '
            group_by += 'po.sale_line_id,'
        if wiz['mrp'] and wiz['mrp_install']:
            ...
            where_location = ' m.location_dest_id IN ' + str(location_tuple) + '  '
        if wiz['pos'] and wiz['pos_install']:
            any_check = True
#             join += ' LEFT JOIN pos_order AS pos ON (p.id = pos.picking_id) '
#             select_type += " WHEN m.picking_id is not null and pos.id is not null THEN 'pos' "
#             group_by += 'pos.id,'
        if wiz['refund']:
            any_check = True
        if wiz['procure']:
        ...
        if wiz['swap']:
            any_check = True
            join += ' LEFT JOIN swap_order_line AS swl ON (po.swap_line_id = swl.id) \
                      LEFT JOIN swap_order AS swo ON (swl.order_id = swo.id) \
                      LEFT JOIN swap_order_out_tax_rel AS stax ON (swo.id = stax.ord_id) '
        ...
        if any_check:
            prod_ids = []
            name_dict = {}
            cr.execute("SELECT cast(m.date as date) AS date, m.origin_returned_move_id AS return_id, "
                            ...
                            "(CASE WHEN m.procurement_id is not null and so.sale_category_id is not null THEN sc.name "
                                ...
                                "WHEN m.picking_id is not null and p.consume_order_id is not null THEN 'consume' "
                                "WHEN m.procurement_id is not null and po.swap_line_id is not null THEN 'swap' "+select_type+" "
                                "WHEN m.origin_returned_move_id is not null THEN 'refund' "
                                "ELSE 'pos' END) AS rep_type, "+select_name+" "
                            "m.product_id AS prod_id, "+select_origin+" "
                            "SUM(coalesce((m.product_qty / u.factor * u2.factor),0)) AS qty, "+select+" "
                            "SUM(coalesce((m.price_unit * m.product_qty / u.factor * u2.factor),0)) AS cost, "+select_price+" "
                            "SUM(coalesce((m.list_price * m.product_qty / u.factor * u2.factor),0)) AS list "
                        "FROM stock_move AS m "
                            ...
                            "LEFT JOIN sale_category AS sc ON (so.sale_category_id = sc.id) "+join+""
                        "WHERE "+where_location+" " 
                            "AND m.state = 'done' "
                            "AND m.date >= %s AND m.date <= %s "+where+" "
                        "GROUP BY m.date,sc.name,so.sale_category_id,m.origin_returned_move_id,m.picking_id,rp.name,m.warehouse_id,sw.partner_id, "+group_by+" "
                            "rp2.name,m.inventory_id,po.swap_line_id,p.name,m.product_id,p.origin,p.consume_order_id,m.procurement_id,p.transit_order_id "
                        "ORDER BY m.date "
                            ,(wiz['date_from'],wiz['date_to']+' 23:59:59'))
            ...
            for r in result:
                ...
                if r['rep_type'] == 'refund' and not wiz['refund']:
                    continue
                if r['rep_type'] == 'consume' and not wiz['consume']:
                    continue
                if r['rep_type'] == 'inventory' and not wiz['inventory']:
                    continue
                if r['rep_type'] == 'swap' and not wiz['swap']:
                    continue
                if not wiz['sales'] and r['rep_type'] not in ('pos','mrp','procure','inventory','consume','swap','refund'):
                    continue
                if r['rep_type'] in ('pos','swap') or r['rep_type'] not in ('mrp','procure','inventory','consume','refund'):
                    if r['rep_type'] == 'pos':
                        tax = round(float(r['amount'] - r['discount'])*float(TAX_FACTOR),4)
                        taxed = (r['amount']-r['discount']) - tax
                ...
                if r['rep_type'] not in res:
                    res[r['rep_type']] = {'name': r['rep_type'],
                                          ...
                                          'pure':0,
                                          'cost':0}
                res[r['rep_type']]['amount'] += r['list']
                res[r['rep_type']]['qty'] += r['qty']
                res[r['rep_type']]['discount'] += r['discount']
                res[r['rep_type']]['total'] += r['amount']
                res[r['rep_type']]['pure'] += r['amount'] - r['discount']
                ...
                if r['name'] not in res[r['rep_type']]['lines']:
                    res[r['rep_type']]['lines'][r['name']] = {'date': r['date'],
                                                              ...
                                                              'cost':0,
                                                              'lines':{}}
                res[r['rep_type']]['lines'][r['name']]['amount'] += r['list']
                res[r['rep_type']]['lines'][r['name']]['qty'] += r['qty']
                res[r['rep_type']]['lines'][r['name']]['discount'] += r['discount']
                res[r['rep_type']]['lines'][r['name']]['total'] += r['amount']
                res[r['rep_type']]['lines'][r['name']]['pure'] += r['amount'] - r['discount']
                ...
                if wiz['type'] == 'detail':
                    if r['prod_id'] not in res[r['rep_type']]['lines'][r['name']]['lines']:
                        res[r['rep_type']]['lines'][r['name']]['lines'][r['prod_id']] = {'date': '',
                                                                                         ...
                                                                                         'pure':0,
                                                                                         'cost':0}
                    res[r['rep_type']]['lines'][r['name']]['lines'][r['prod_id']]['amount'] += r['list']
                    res[r['rep_type']]['lines'][r['name']]['lines'][r['prod_id']]['qty'] += r['qty']
                    res[r['rep_type']]['lines'][r['name']]['lines'][r['prod_id']]['discount'] += r['discount']
                    res[r['rep_type']]['lines'][r['name']]['lines'][r['prod_id']]['total'] += r['amount']
                    res[r['rep_type']]['lines'][r['name']]['lines'][r['prod_id']]['pure'] += r['amount'] - r['discount']
                ...
                total_dict['amount'] += r['list']
                total_dict['qty'] += r['qty']
                total_dict['cost'] += r['cost']
                total_dict['tax'] += tax
                total_dict['taxed'] += taxed
                total_dict['total'] += r['amount']
                total_dict['discount'] += r['discount']
                total_dict['pure'] += r['amount'] - r['discount']
        row = ['','','','', u'<b><c>НИЙТ ДҮН</c></b>']
        row += ['<b>%s</b>' % total_dict['qty'],
                '<b>%s</b>' % total_dict['amount'],
                '<b>%s</b>' % total_dict['total'],
                '<b>%s</b>' % total_dict['discount'],
                '<b>%s</b>' % total_dict['pure'],
                '<b>%s</b>' % total_dict['tax'],
                '<b>%s</b>' % total_dict['taxed']]
        if wiz['cost']:
        ...
        if res:
            ...
            for val in sorted(res.values(), key=itemgetter('name')):
                ...
                if val['name'] == 'procure':
                    ...
                    name = u'Посын борлуулалт'
                elif val['name'] == 'mrp':
                    name = u'Үйлдвэрлэл'
                elif val['name'] == 'swap':
                    name = u'Солилцоо'
                elif val['name'] == 'consume':
                    name = u'Дотоод зарлага'
                ...
                for v in sorted(val['lines'].values(), key=itemgetter('date')):
                    if wiz['type'] == 'detail':
                        ...
                        row += ['<b>%s</b>'%v['qty'],
                                '<b>%s</b>'%v['amount'],
                                '<b>%s</b>'%v['total'],
                                '<b>%s</b>'%v['discount'],'<b>%s</b>'%v['pure'],
                                '<b>%s</b>'%v['tax'],'<b>%s</b>'%v['taxed']]
                        if wiz['cost']:
                            row += ['<b>%s</b>'%v['cost']]
                    else:
                        row = ['<c><str>%s</str></c>'%str(number),'<c>%s</c>'%v['date'],
                               '%s'%v['name'],'%s'%v['origin'],
                               u'%s'%v['partner']]
                        row += [v['qty'],v['amount'],v['total'],v['discount'],v['pure'],
                                v['tax'],v['taxed']]
                        if wiz['cost']:
                            row += [v['cost']]
                    number += 1
                    ...
                    if wiz['type'] == 'detail':
                        for p in sorted(v['lines'].values(), key=itemgetter('name')):
                            row = ['','','','',u'%s'%p['name'],p['qty'],p['amount'],p['total'],
                                   p['discount'],p['pure'],p['tax'],p['taxed']]
                            if wiz['cost']:
                                row += [p['cost']]
                            data.append(row)
        return data, titles

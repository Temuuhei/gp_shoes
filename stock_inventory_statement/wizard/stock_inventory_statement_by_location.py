...
class stock_inventory_statement_by_location(osv.osv_memory):
    _name = 'stock.inventory.statement.by.location'
    ...
    _columns = {
        ...
        'warehouse_ids': fields.many2many('stock.warehouse', 'stock_inventory_statement_warehouse_rel2', 'wizard_id', 'warehouse_id', 'Warehouse'),
        'location_ids': fields.many2many('stock.location', 'stock_inventory_statement_location_rel', 'wizard_id', 'location_id', 'Location'),
    ...
    def onchange_warehouse_ids(self, cr, uid, ids, warehouse_ids, context=None):
        location_ids = []
        selected_warehouse_ids = []
        if warehouse_ids:
            for warehouse_id in warehouse_ids:
                selected_warehouse_ids = warehouse_id[-1]
#                 print '\n\nWAREHOUSE IDS::',warehouse_id[-1]
                for selected_id in selected_warehouse_ids:
                    loc_ids = []
                    warehouse = self.pool.get('stock.warehouse').browse(cr, uid, selected_id)
                    loc_ids = self.pool.get('stock.location').search(cr, uid, [('location_id', '=', warehouse.lot_stock_id.id)])
    ...
    def prepare_report_data(self, cr, uid, wiz, context=None):
        print "CALLED prepare_report_data()::", wiz
        if context is None:
            context = {}
        ...
        if wiz['income_expense']:
            ...
            loc_names = u""
            for item in wiz['location_ids']:
                location = self.pool.get('stock.location').browse(cr, uid, item)
                loc_names += u",%s" % (location.name)

            titles.append(u'Байрлал: %s' % loc_names) 
            ...
            for wh in warehouses:
                ...
                if wiz['location_ids']:
                    locations = tuple(wiz['location_ids'])
                    print "LOCATIONS SUCCESFULLY::", locations,
                else:

...
class PlmComponent(models.Model):
    ...
    def on_change_name(self):
        if self.name:
            ...
            if not results:
                results = self.search([('name', '=', self.name), ('active', '=', False)])
            if len(results) > 0:
...
#   Internal methods
    ...
    def SaveOrUpdate(self, vals):
        ...
        for partVals in vals:
            ...
            if 'engineering_code' not in partVals:
                ...
                partVals['hasSaved'] = hasToBeSaved
            ...
            for existingCompBrws in existingCompBrwsList:
                if not hasToBeSaved:    # Component exists in Odoo
    ...
    def _get_recursive_parts(self, excludeStatuses, includeStatuses):
        ...
        for prodBrws in self:
            logging.info('Getting recursive parts from product %r' % (prodBrws))
    ...
    def action_confirm(self):
        ...
        """
        logging.info('Going to confirm Components %r' % (self))
        for compObj in self:
            defaults = {}
            status = 'confirmed'
            action = 'confirm'
    ...
    def _action_to_perform(self, status, action, docaction, defaults=[], excludeStatuses=[], includeStatuses=[]):
        logging.info('Going to perform action on %r' % (self))
        product_product_ids = []
        ...
        if action:
            self.browse(product_product_ids).signal_workflow(action)
